...
def file_write(list_name, outcome_list):
    global Progress_File
    for i in range(0, len(outcome_list), 3):
        small_list = outcome_list[i:i + 3]
        Progress_File.write(f'\n{list_name}')
        small_list2 = [str(x) for x in small_list]
        Progress_File.write(', '.join(small_list2))  # Learned how to write a list without brackets from here :- https://stackoverflow.com/questions/11178061/print-list-without-brackets-in-a-single-row


Progress_File = open('ProgressList.txt', 'w')
Progress_File.close()
...
print('\nVertical Histogram\n')
header = ['  Progress', str(progress), '| Trailing', str(trailer), '| Retriever', str(retriever), '| Excluded',
          str(exclude)]
print(' '.join(header))
...
Progress_File = open('ProgressList.txt', 'a')

file_write('  Progress - ', progress_list)
file_write('  Progress (module trailer) - ', trailer_list)
file_write('  Module retriever - ', retriever_list)
file_write('  Exclude – ', exclude_list)

Progress_File = open('ProgressList.txt', 'r')
